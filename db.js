// ─────────────────────────────────────────────────────────────────────
// SMARTCOT - BASE DE DATOS COMPLETA (CON MIGRACIÓN)
// ─────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────
// ESQUEMA DE BASE DE DATOS (VERSIÓN 2)
// ─────────────────────────────────────────────────────────────────────
db.version(2).stores({
...
async function dbErrorHandler(operation, error) {
    ...
    // Si es error de versión, intentar reconstruir la DB
...
window.dbCotizaciones = {
    guardar: async function(cotizacion) {
        try {
            ...
            return dbErrorHandler('guardar cotización', error);
        }
    },
    ...
    obtener: async function(id) {
        try {
            ...
            return dbErrorHandler('obtener cotización', error);
        }
    },
...
window.dbClientes = {
    guardar: async function(cliente) {
        try {
            ...
            return dbErrorHandler('guardar cliente', error);
        }
    },
    ...
    obtener: async function(id) {
        try {
            ...
            return dbErrorHandler('obtener cliente', error);
        }
    },
...
window.dbConfig = {
    guardar: async function(clave, valor) {
        try {
            ...
            return dbErrorHandler('guardar configuración', error);
        }
    },
    ...
    obtenerTodas: async function() {
        try {
            ...
            return { iva: 16, utilidad: 10, empresa: '' };
        }
    }
};
...
window.dbMateriales = {
    buscar: async function(termino, limite = 50) {
        try {
            ...
            const resultados = await db.materiales
                .filter(m => m.nombre.toLowerCase().includes(termino.toLowerCase()))
                .limit(limite)
                .toArray();
            return resultados;
        } catch (error) {
    ...
    guardar: async function(material) {
        try {
            return await db.materiales.put(material);
        } catch (error) {
            return dbErrorHandler('guardar material', error);
        }
    },
    ...
    importarBulk: async function(materiales) {
        try {
            ...
            return dbErrorHandler('importar materiales', error);
        }
    }
};
...
window.dbManoObra = {
    ...
    guardar: async function(mo) {
        try {
            return await db.manoObra.put(mo);
        } catch (error) {
            return dbErrorHandler('guardar mano de obra', error);
        }
    }
};
...
window.dbMaquinaria = {
    ...
    guardar: async function(maq) {
        try {
            return await db.maquinaria.put(maq);
        } catch (error) {
            return dbErrorHandler('guardar maquinaria', error);
        }
    }
};
...
window.dbFactores = {
    ...
    guardar: async function(factor) {
        try {
            return await db.factoresRendimiento.put(factor);
        } catch (error) {
            return dbErrorHandler('guardar factor', error);
        }
    }
};
...
window.dbImportar = async function(datosJSON) {
    try {
        ...
        if (datos.cotizaciones) await db.cotizaciones.bulkAdd(datos.cotizaciones);
        if (datos.clientes) await db.clientes.bulkAdd(datos.clientes);
        if (datos.configuracion) await db.configuracion.bulkPut(datos.configuracion);
...
window.dbEstadisticas = async function() {
    try {
        ...
        return { cotizaciones: 0, clientes: 0, totalIngresos: 0, margenPromedio: 0 };
    }
};
...
window.dbInicializarEjemplo = async function() {
    try {
        const factoresExistentes = await db.factoresRendimiento.count();
        if (factoresExistentes === 0) {
            await db.factoresRendimiento.bulkAdd([
                { tipo: 'altura', nombre: 'Altura >3m', valor: 1.15, descripcion: '+15%' },
                { tipo: 'dificultad', nombre: 'Espacio confinado', valor: 1.20, descripcion: '+20%' },
                { tipo: 'desperdicio', nombre: 'Desperdicio estándar', valor: 1.05, descripcion: '+5%' }
            ]);
        }
        ...
        if (clientesExistentes === 0) {
            await db.clientes.bulkAdd([
                { nombre: 'Cliente Ejemplo 1', email: 'cliente1@email.com', telefono: '55-1234-5678' },
                { nombre: 'Cliente Ejemplo 2', email: 'cliente2@email.com', telefono: '55-8765-4321' }
            ]);
        }

        console.log('✅ Datos de ejemplo inicializados');
    } catch (error) {
        console.error('⚠️ Error inicializando ejemplos:', error);
    }
};
