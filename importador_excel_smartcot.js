...
window.importadorSmartCot = {
    ...
    // ─────────────────────────────────────────────────────────────────────
    // FUNCIONES DE MANEJO DE ARCHIVOS (AGREGAR ESTO)
    // ─────────────────────────────────────────────────────────────────────
    
    // Manejar selección de archivo desde input file
    ...
    handleDrop: function(event) {
        console.log('📁 handleDrop llamado');
        event.preventDefault();
        event.stopPropagation();
        
        // Resetear estilo del drop zone
    ...
    // Manejar drag over (para efecto visual)
    handleDragOver: function(event) {
        ...
        if (dropZone) {
            ...
            dropZone.style.background = 'var(--blue-l)';
        }
    },
    
    // Manejar drag leave (para resetear estilo)
    ...
    // ─────────────────────────────────────────────────────────────────────
    // PROCESAR ARCHIVO (FUNCIÓN AUXILIAR)
    // ─────────────────────────────────────────────────────────────────────
    procesarArchivo: async function(file) {
        try {
            ...
            if (dropZone) {
                dropZone.innerHTML = `
                    ...
                    <div style="font-size:17px;font-weight:700;color:var(--green);margin-bottom:8px;">
                        ${file.name}
                    </div>
                    ...
                    <button type="button" onclick="document.getElementById('excel-file').click();" 
                        style="padding:12px 28px;background:var(--bg);color:var(--ink2);border:1px solid var(--border);border-radius:var(--radius-sm);font-size:14px;font-weight:600;cursor:pointer;font-family:var(--font);">
                        📁 Cambiar archivo
                    </button>
                `;
            }
            ...
            // Iniciar importación
            await this.iniciarImportacionConArchivo(file);
    ...
    // ─────────────────────────────────────────────────────────────────────
    // INICIAR IMPORTACIÓN CON ARCHIVO (CORREGIDO)
    // ─────────────────────────────────────────────────────────────────────
    iniciarImportacionConArchivo: async function(file) {
        try {
            // Verificar licencia
            const limite = await window.licencia.verificarLimite('importar');
            if (!limite.permitido) {
                alert('❌ ' + limite.razon);
                return;
            }
            ...
            // Obtener límite de conceptos según plan
